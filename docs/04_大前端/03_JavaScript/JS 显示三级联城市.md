...
![image-20200605120320927](https://jy-imgs.oss-cn-beijing.aliyuncs.com/img/image-20200605120320927.png)
...
地址：https://janycode.github.io/files/proCity.js

#### 2. 省市县数据(172k)

地址：https://janycode.github.io/files/proCityCountry.js

#### 3. 省市县乡数据(1545k)

地址：https://janycode.github.io/files/proCityCountryTown.js

以下把各个数据整理成单独文件，方便组合使用：

#### 4. 省(1k)

地址：https://janycode.github.io/files/pro.js

#### 5. 市(14k)

地址：https://janycode.github.io/files/city.js

#### 6. 县(159k)

地址：https://janycode.github.io/files/country.js

#### 7. 乡镇数据(1373k)

地址：https://janycode.github.io/files/town.js
...
<html>
	...
	<script type="text/javascript">
		var city = /* 三级联城市 JSON 字符串(本文下方) */;
		
		var citys = JSON.parse(city);
		console.log(citys); // 转 js 对象
		console.log(citys.citylist[4].p); // eg:获取省份
		console.log(citys.citylist[4].c[0].n) // eg:获取城市
		console.log(citys.citylist[4].c[0].a[0].s) // eg:获取区/县
        //var provName = citys.citylist[province.selectedIndex].p;
		//var cityName = citys.citylist[province.selectedIndex].c[city.selectedIndex].n;
		//var counName = citys.citylist[province.selectedIndex].c[city.selectedIndex].a[county.selectedIndex].s;
		...
		// 清空"省份"
		// var provChild = province.childNodes;
		// for (var i = 0; i < provChild.length;) {
		// 	province.removeChild(provChild[i]);
		// }
		// 页面加载完默认先把省份添加
		var provArr = citys.citylist;
		for (var i = 0; i < provArr.length; i++) {
			var op = document.createElement("option");
			op.innerHTML = provArr[i].p;
		...
		function showCity() {
			...
			for (var i = 0; i < cityChild.length;) {
				city.removeChild(cityChild[i]);
			}
			
			// 添加城市 到 对应省份
			var cityArr = citys.citylist[province.selectedIndex].c;
			for (var i = 0; i < cityArr.length; i++) {
				var op = document.createElement("option");
				op.innerHTML = cityArr[i].n;
				city.appendChild(op);
			}
		...
		function showCounty() {
			// 清空上一个选中的级联区县
			var countyChild = county.childNodes;
			for (var i = 0; i < countyChild.length;) {
				county.removeChild(countyChild[i]);
			}
			
			// 添加区县 到 对应城市
			var countyArr = citys.citylist[province.selectedIndex].c[city.selectedIndex].a;
			for (var i = 0; i < countyArr.length; i++) {
				var op = document.createElement("option");
				op.innerHTML = countyArr[i].s;
...
<head>
	<meta charset="utf-8">
	<title>省市区县镇级联数据JS版</title>
</head>
<body>
	<div id="main" style="width:1000px;margin:30px auto;">
		<h2 style="text-align:center">省市区县镇级联数据JS版</h2>
		<p><span style="font-weight:bold">前言</span>：做专题经常会涉及到报名，报名通常就会有地区级联的要求，如下所示。因为销售提供的数据都不完整或者干脆都不提供，在网上也找不到现成的数据。昨天鼓捣出一份数据，分享下。</p>
		<p style="font-weight:bold">ps：如果数据的名称与文件中已有的名称冲突，请自行修改。</p>
		<p>
			<div>
				<select id="proS" onchange="loadCity()">
					<option value="0">请选择</option>
				</select>
				<select id="cityS" onchange="loadCounty()">
					<option value="0">请选择</option>
				</select>
				<select id="countryS" onchange="loadTown()">
					<option value="0">请选择</option>
				</select>
				<select id="townS">
					<option value="0">请选择</option>
		...
		<h3>1、省市数据(15k)</h3>
		<p>地址：<a target="_blank" href="http://www1.pcauto.com.cn/zt/pcauto20160111/areaData/proCity.js">http://www1.pcauto.com.cn/zt/pcauto20160111/areaData/proCity.js</a></p>
		<h3>1、省市县数据(172k)</h3>
		<p>地址：<a target="_blank" href="http://www1.pcauto.com.cn/zt/pcauto20160111/areaData/proCityCountry.js">http://www1.pcauto.com.cn/zt/pcauto20160111/areaData/proCityCountry.js</a></p>
		<h3>2、省市县乡数据(1545k)</h3>
		<p>地址：<a target="_blank" href="http://www1.pcauto.com.cn/zt/pcauto20160111/areaData/proCityCountryTown.js">http://www1.pcauto.com.cn/zt/pcauto20160111/areaData/proCityCountryTown.js</a></p>
		<p style="font-weight:bold">以下把各个数据整理成单独文件，方便组合使用</p>
		<h3>3、省(1k)</h3>
		<p>地址：<a target="_blank" href="http://www1.pcauto.com.cn/zt/pcauto20160111/areaData/pro.js">http://www1.pcauto.com.cn/zt/pcauto20160111/areaData/pro.js</a></p>
		<h3>4、市(14k)</h3>
		<p>地址：<a target="_blank" href="http://www1.pcauto.com.cn/zt/pcauto20160111/areaData/city.js">http://www1.pcauto.com.cn/zt/pcauto20160111/areaData/city.js</a></p>
		<h3>5、县(159k)</h3>
		<p>地址：<a target="_blank" href="http://www1.pcauto.com.cn/zt/pcauto20160111/areaData/country.js">http://www1.pcauto.com.cn/zt/pcauto20160111/areaData/country.js</a></p>
		<h3>6、乡镇数据(1373k)</h3>
		<p>地址：<a target="_blank" href="http://www1.pcauto.com.cn/zt/pcauto20160111/areaData/town.js">http://www1.pcauto.com.cn/zt/pcauto20160111/areaData/town.js</a></p>
		<p style="color:red">当级别到乡的时候数据已经很大了，有1m多，不应该把js文件一次性加载进来，这样会拖慢页面的加载时间，采用分次加载比较好。比如先加载proCityCountry.js,页面加载完之后在加载town.js；当然这仅仅是初步的优化，还可以优化更多。以上仅仅是参考意见，各位示情况而用。</p>
		<!-- 示例程序  -->
		<script src="http://www1.pcauto.com.cn/zt/pcauto20160111/areaData/proCityCountry.js"></script>
		<script async src="http://www1.pcauto.com.cn/zt/pcauto20160111/areaData/town.js"></script>
		<script type="text/javascript">
			//非盈利性分享，如果侵犯了你的权益，请知会马上删除。希望多多支持
